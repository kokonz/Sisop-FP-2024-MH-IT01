...
void *handle_client(void *arg) {
    ...
    while ((valread = read(client_socket, buffer, BUF_SIZE)) > 0) {
        buffer[valread] = '\0';
        char command[BUF_SIZE], password[BUF_SIZE], channel[BUF_SIZE], key[BUF_SIZE], new_channel[BUF_SIZE], room[BUF_SIZE], new_room[BUF_SIZE], user_to_ban[BUF_SIZE], user_to_unban[BUF_SIZE], text[BUF_SIZE];
        ...
        if (sscanf(buffer, "REGISTER %s -p %s", username, password) == 2) {
            ...
            if (strcmp(buffer, "LIST CHANNEL") == 0) {
                ...
                if (join_channel(client_socket, username, channel, NULL)) {
            ...
            } else if (sscanf(buffer, "BAN %s", username) == 1) {
                ban_user(client_socket, logged_in_user, username, current_channel);
            } else if (sscanf(buffer, "UNBAN %s", user_to_unban) == 1) {
                unban_user(client_socket, logged_in_user, username, current_channel);
...
void delete_all_rooms(int client_socket, const char *username, const char *channel) {
    ...
    char msg[] = "Semua room berhasil dihapus\n";
    write(client_socket, msg, strlen(msg));
...
int edit_user(int client_socket, const char *old_username, const char *new_username, int is_self_edit) {
    ...
    fclose(user_file);
    printf("Debug: logged_in_user: %s, role: %s\n", logged_in_user, logged_in_user_role);
    fflush(stdout);
    ...
    while (fgets(line, sizeof(line), user_file)) {
        ...
        if (strcmp(stored_username, old_username) == 0) {
            user_found = 1;
            fprintf(temp_file, "%d,%s,%s\n", user_id, new_username, stored_password);
        } else {
...
void edit_user_password(int client_socket, const char *username, const char *new_password, int is_self_edit) {
    ...
    while (fgets(line, sizeof(line), user_file)) {
        ...
        if (strcmp(stored_username, username) == 0) {
            ...
            fprintf(temp, "%d,%s,%s,%s\n", id, stored_username, new_password, role);
        } else {
            fputs(line, temp);
        }
    }
...
void edit_chat(int client_socket, const char *username, const char *channel, const char *room, int chat_id, const char *text) {
    ...
    char temp_file[] = "/home/kokon/FP/DiscorIT/chat_temp.csv";
    FILE *temp = fopen(temp_file, "w");
    if (!temp) {
        perror("fopen");
        fclose(file);
    ...
    while (fgets(line, sizeof(line), file)) {
        int id;
        char timestamp[20], user[BUF_SIZE], message[BUF_SIZE];
        sscanf(line, "[%19[^]]][%d][%[^]]] \"%[^\"]\"", timestamp, &id, user, message);

        if (id == chat_id) {
            fprintf(temp, "[%s][%d][%s] \"%s\"\n", timestamp, id, user, text);
            chat_found = 1;
        } else {
            fputs(line, temp);
        }
    ...
    if (chat_found) {
        rename(temp_file, chat_file_path);

        char msg[BUF_SIZE];
        snprintf(msg, sizeof(msg), "Pesan berhasil diubah\n");
        write(client_socket, msg, strlen(msg));

        char activity[BUF_SIZE];
        snprintf(activity, sizeof(activity), "%s mengubah pesan di room %s channel %s: %s", username, room, channel, text);
        log_activity(channel, activity);
    } else {
        remove(temp_file);
        char msg[BUF_SIZE];
        snprintf(msg, sizeof(msg), "Pesan tidak ditemukan\n");
        write(client_socket, msg, strlen(msg));
    }
}
...
void del_chat(int client_socket, const char *username, const char *channel, const char *room, int chat_id) {
    ...
    if (chat_found) {
        ...
        char msg[BUF_SIZE];
        snprintf(msg, sizeof(msg), "Pesan berhasil dihapus\n");
        write(client_socket, msg, strlen(msg));

        char activity[BUF_SIZE];
        snprintf(activity, sizeof(activity), "%s menghapus pesan di room %s channel %s", username, room, channel);
        log_activity(channel, activity);
    } else {
        remove(temp_file);
        char msg[BUF_SIZE];
...
void initialize_directories() {
    ...
    if (stat("/home/kokon/FP/DiscorIT", &st) == -1) {
